...
import { useRuns, useJobs } from "@/lib/hooks/use-api"
...
export default function DashboardPage() {
  ...
  const totalSubmitted = runs?.reduce((acc, r) => acc + r.submitted_tasks, 0) || 0
  const totalTasks = runs?.reduce((acc, r) => acc + r.total_tasks, 0) || 0
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {jobs?.filter((j) => !j.has_been_applied_to).length || 0} not applied
            </p>
          </CardContent>
        </Card>
